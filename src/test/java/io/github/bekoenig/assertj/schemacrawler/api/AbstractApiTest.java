...
public abstract class AbstractApiTest {
    ...
    protected boolean shouldBeCovered(Method method) {
    ...
    protected List<String> getCandidates(Class<?> scClass, Method scMethod) {
    ...
    protected Set<String> getAllMethods(Class<?> clazz) {
    ...
    protected Map<String, Boolean> getAllMethodsWithDeprecation(Class<?> clazz) {
        Map<String, Boolean> methods = new HashMap<>();
        Class<?> current = clazz;
        while (current != null && current != Object.class) {
            for (Method m : current.getDeclaredMethods()) {
                methods.putIfAbsent(m.getName(), m.isAnnotationPresent(Deprecated.class));
            }
            current = current.getSuperclass();
        }
        return methods;
